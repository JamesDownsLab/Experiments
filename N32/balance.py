...
class Balancer:
    ...
    def balance(self, repeats=5, threshold=10):
        ...
        while balanced is False:
            ...
            annotated_im = self.annotate_image(im, center, mean_center, distance, centers)
    ...
    def get_center(self, im):
        circles = images.find_circles(im, 11, 235, 3, 6, 7)
        colors = []
        for x, y, r, in circles:
            x, y, r = int(x), int(y), int(r)
            circle = im[y-r:y+r, x-r:x+r]
            colors.append(np.mean(circle))
        colors = np.array(colors)
        circles = circles[colors<100, :]
